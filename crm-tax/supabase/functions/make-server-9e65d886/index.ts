...
app.post("/make-server-9e65d886/signup", async (c) => {
  try {
    ...
    const body = await c.req.json();
    const { username, password, email, tenantSlug } = body;
    console.log('[SIGNUP] Received data:', { username, email, tenantSlug, passwordLength: password?.length });
    ...
    try {
      ...
      // If this user belongs to a tenant and the tenant has no owner yet, make them the owner
      if (tenantSlug) {
        const tenant = await kv.get(`tenant:${tenantSlug}`);
        if (tenant && !tenant.ownerUsername) {
          await kv.set(`tenant:${tenantSlug}`, { ...tenant, ownerUsername: username });
          console.log(`[SIGNUP] Set "${username}" as owner of tenant "${tenantSlug}"`);
        }
      }
